...
mod tests {
    ...
    fn pretty(f: &Flags) -> String {
        ...
        let s = r.fold(String::new(), |mut a, s| {a.extend(s.chars()); a} );

        format!("{}, shrink={}, {}", hex::encode(f.flags.to_be_bytes()), f.shrink, s)
    }
    ...
    fn extend_eq_shrink() {
        ...
        for i in 1..65 {
            println!("{:?}", pretty(&flags));
            assert_eq!(flags.pop_flag(), Flags::LAMBDA_FLAG, "test popping after {}th time", i);
        }
